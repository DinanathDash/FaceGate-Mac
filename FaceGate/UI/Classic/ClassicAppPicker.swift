...
/// View for selecting which installed apps to lock (Classic UI).
/// Shows a searchable grid of installed apps with toggle switches.
struct ClassicAppPicker: View {
            ...
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + remainingDelay) {
                    withAnimation(.spring(response: 0.18, dampingFraction: 0.8, blendDuration: 0)) {
                        self.lockedAppsManager.lockApp(lockedApp)
                    }
                }
            }
        }
...
private struct ClassicAppRow: View {
